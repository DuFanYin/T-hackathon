...
Monitors all trading pairs by default; override via setting symbol or symbols for a subset.
"""
...
class Strat1Pine(StrategyTemplate):
    ...
    def on_init_logic(self) -> None:
        self.write_log(
            f"Strat1Pine symbols={len(self.symbols)} pivot_len={self.pivot_len} rr={self.rr} use_limit={self.use_limit}",
            level="INFO",
        )
    ...
    def on_timer_logic(self) -> None:
        ...
        if not market:
            self.write_log("Strat1 no market_engine", level="WARN")
            return
    ...
    def _tick_symbol(self, market: Any, symbol: str, interval: str) -> None:
        ...
        if len(last_bars) < 4:
            ...
            if bar_count > 0 and bar_count % 100 == 1:
                self.write_log(f"Strat1 {symbol} waiting for bars: have {len(last_bars)}/4", level="DEBUG")
            return
        ...
        if not flat and st["active_stop"] is not None and st["active_target"] is not None:
            ...
            if last is not None:
                if last <= st["active_stop"]:
                    ...
                    self.write_log(f"Strat1 {symbol} exit stop hit {last} <= {st['active_stop']}", level="WARN")
                    st["active_stop"] = None
                    st["active_target"] = None
                elif last >= st["active_target"]:
                    self.send_order(symbol, "SELL", abs(position_size), last, "MARKET")
                    self.write_log(f"Strat1 {symbol} exit target hit {last} >= {st['active_target']}", level="INFO")
                    st["active_stop"] = None
                    st["active_target"] = None
        ...
        if self.use_limit and flat and pending_ids and bar_count - st["limit_bar_idx"] >= self.fill_bars + 1:
            ...
            self.write_log(f"Strat1 {symbol} limit timeout, cancel {oid}", level="WARN")
        ...
        if pl is not None:
            ...
            st["hit_count"] = 0
            st["hit1_low"] = None
            self.write_log(f"Strat1 {symbol} pivot low={pl:.2f} support updated", level="DEBUG")
        ...
        if st["hit_count"] == 1 and ok_bear and ok_t3 and ok_cs and ok_po and ok_atr:
            ...
            self.write_log(f"Strat1 {symbol} H2 higher-low fail, reset support", level="DEBUG")
            st["sup_price"] = None
            st["hit_count"] = 0
            st["hit1_low"] = None
        ...
        if signal and flat:
            ...
            if self.use_limit:
                ...
                self.write_log(f"Strat1 {symbol} limit BUY @ {entry_price:.2f} qty={self.quantity}", level="INFO")
            else:
                self.send_order(symbol, "BUY", self.quantity, entry_price, "MARKET")
                self.write_log(f"Strat1 {symbol} market BUY qty={self.quantity}", level="INFO")
            st["active_stop"] = stop_price
            st["active_target"] = target_price
            self.write_log(
                f"Strat1 {symbol} signal entry={entry_price} stop={stop_price} target={target_price} rr={self.rr}",
                level="INFO",
            )
